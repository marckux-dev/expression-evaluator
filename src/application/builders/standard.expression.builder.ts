...
export class StandardExpressionBuilder extends ExpressionBuilder {
  ...
  private static endsOperand(token: TokenInterface): boolean {
    return (
      ...
      (token instanceof OperatorEntity &&
  ...
  private static isNumberLiteral(token: TokenInterface): boolean {
    return (
      token instanceof ConstantEntity && token.constructor === ConstantEntity
    );
  }
  ...
  public manageImplicitMultiplication(): this {
    for (let i = 0; i < this.tokens.length - 1; i++) {
      ...
      if (
        next instanceof ConstantEntity &&
        StandardExpressionBuilder.isNumberLiteral(next)
      ) {
        ...
        (next instanceof OperatorEntity &&
          next.position === OperatorPosition.PREFIX)
      ) {
        this.tokens.splice(i + 1, 0, new ImplicitMultiplicationOperator());
      }
    }
      ...
      if (token instanceof SubtractionOperator) {
        if (
          ...
          (this.tokens[i - 1] instanceof OperatorEntity &&
            !(
              (this.tokens[i - 1] as OperatorEntity).position ===
  ...
  public toRpn(): this {
    ...
    while (token) {
      ...
      if (token instanceof ControllerEntity) {
        ...
        if (level < 0) {
          throw new InvalidExpressionError(
            `Error near ${token.getSymbol()}. Check the brackets.`
          );
        }
        ...
        while (top && top.getPrecedence() >= token.getPrecedence()) {
          if (
            ...
            top.associativity === OperatorAssociativity.RIGHT
          ) {
            break;
          }
          stack.push(top);
